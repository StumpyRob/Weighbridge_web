...
def test_destination_form_shows_qz_printer_controls_but_not_backend_signing_settings(client):
...
def test_destination_create_saves_qz_printer_settings_from_form_fields(client, db_session):
    ticket_template = _create_template(
        db_session,
        code="ADMIN_TICKET_QZ_TEMPLATE",
        document_type="TICKET",
        template_format="TEXT",
        content="{{ payload.ticket_no }}",
    )
    ...
    response = client.post(
        "/admin/printing/destinations/new",
        data={
            "name": "Ticket QZ Field Destination",
            "description": "Field configured",
            "document_type": "TICKET",
            "template_id": str(ticket_template.id),
            "delivery_type": "PRINT_LOCAL_BROWSER",
            "qz_direct_print_enabled": "1",
            "qz_printer_name": "Office Ticket Printer",
            "is_default": "1",
            "is_active": "1",
        },
        follow_redirects=False,
    ...
    destination = db_session.execute(
        select(PrintDestination).where(PrintDestination.name == "Ticket QZ Field Destination")
    ).scalars().first()
    assert destination is not None
    assert destination.delivery_type == "PRINT_LOCAL_BROWSER"
    assert destination.delivery_config == {
    ...
    assert 'id="qz_direct_print_enabled"' in edit_page.text
    assert 'id="qz_printer_name"' in edit_page.text
    assert 'value="Office Ticket Printer"' in edit_page.text


def test_destination_create_keeps_qz_json_compatibility(client, db_session):
    ticket_template = _create_template(
        db_session,
        code="ADMIN_TICKET_QZ_JSON_TEMPLATE",
        document_type="TICKET",
        template_format="TEXT",
        content="{{ payload.ticket_no }}",
    )
    ...
    response = client.post(
        "/admin/printing/destinations/new",
        data={
            "name": "Ticket QZ JSON Destination",
            "description": "JSON configured",
            "document_type": "TICKET",
            "template_id": str(ticket_template.id),
            "delivery_type": "PRINT_LOCAL_BROWSER",
            "delivery_config": '{\n  "qz_direct_print_enabled": true,\n  "qz_printer_name": "Legacy JSON Printer"\n}',
            "is_default": "1",
            "is_active": "1",
        },
        follow_redirects=False,
    ...
    destination = db_session.execute(
        select(PrintDestination).where(PrintDestination.name == "Ticket QZ JSON Destination")
    ).scalars().first()
    assert destination is not None
    assert destination.delivery_config.get("qz_direct_print_enabled") is True
    assert destination.delivery_config.get("qz_printer_name") == "Legacy JSON Printer"
