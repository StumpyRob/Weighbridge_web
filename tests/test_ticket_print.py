...
def test_ticket_direct_print_node_http_route_sends_expected_payload_and_persists_metadata(
    client,
    db_session,
    monkeypatch,
):
    ...
    ticket = _create_complete_sale_ticket(db_session, ticket_no="T-NODE-PRINT-1")
    _set_ticket_node_http_destination(
        db_session,
        code="TICKET_NODE_HTTP_USER",
    ...
    def _fake_post(url, *, json, headers, timeout):
        ...
        return transport_service.httpx.Response(
            ...
            json={
                "ok": True,
                "provider_job_ref": "ticket-route-agent-1",
                "message": "Accepted",
            },
        )
    ...
    assert called["json"] == {
        ...
        "payload_base64": "VGlja2V0IFQtTk9ERS1QUklOVC0x",
    }
    assert job.status == "SENT"
...
def test_ticket_direct_print_node_http_route_marks_failed_and_persists_provider_failure(
    client,
    db_session,
    monkeypatch,
):
    ...
    ticket = _create_complete_sale_ticket(db_session, ticket_no="T-NODE-PRINT-FAIL-1")
    _set_ticket_node_http_destination(
        db_session,
        code="TICKET_NODE_HTTP_FAIL",
        content="Ticket {{ payload.ticket_no }}",
    ...
    def _fake_post(_url, *, json, headers, timeout):
        ...
        return transport_service.httpx.Response(
            ...
            json={
                "ok": False,
                "provider_job_ref": "ticket-route-agent-fail-1",
                "message": "Printer 'Missing Printer' was not found",
            },
        )
    ...
    assert response.status_code == 303
    assert "print_failed=1" in response.headers["location"]
    job = (
        db_session.query(PrintJob)
        .filter(PrintJob.ticket_id == ticket.id, PrintJob.document_type == "TICKET")
        .order_by(PrintJob.id.desc())
    ...
    assert job.payload_format == "TEXT"
    assert job.payload_mime_type == "text/plain; charset=utf-8"
    assert job.provider_job_ref == "ticket-route-agent-fail-1"
    assert job.provider_response_json == {
        "ok": False,
        "provider_job_ref": "ticket-route-agent-fail-1",
        "message": "Printer 'Missing Printer' was not found",
    ...
    assert (
        job.last_error
        == "Print node rejected job: Printer 'Missing Printer' was not found"
    )
